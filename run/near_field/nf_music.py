...
d0 = wavelength / 2
power_source = 1.0
power_noise = 0.1 # SNR = 0 dB
n_snapshots = 300

# Create a 12-element ULA
ula = UniformLinearArray(9, d0)

distance = np.array([5, 10])
angle = np.array([-30, 60]) * np.pi / 180
...
grid_start = (0, -np.pi / 2)
grid_stop = (40, np.pi / 2)
grid = NearField2DSearchGrid(start=grid_start, stop=grid_stop, size=(181, 401))
...
sp = 10 * np.log10(abs(sp)/np.max(abs(sp)))
...
doaplot.plot_spectrum(sp, grid, ground_truth=sources,
                      use_log_scale=False, plot_2d_mode='surface',
